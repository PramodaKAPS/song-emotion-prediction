...
def load_and_prepare_data(drive_folder):
    # Download NLTK resources (moved here for modularity)
    ...
    # Use full 'train' split for Emobank; full Spotify for predictions
    if not sentence_df.empty:
        sentence_df = sentence_df[sentence_df['split'] == 'train']  # Full 'train' split
        print(f"Emobank rows: {len(sentence_df)}")  # Debug print
    if not song_df.empty:
        print(f"Full Spotify rows for predictions: {len(song_df)}")  # Debug print
    ...
    # Calculate linear arousal/valence
