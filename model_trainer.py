...
from utils import assign_quadrant  # Import from utils
...
def train_and_evaluate_models(sentence_embeddings, sentence_df, drive_folder):
    if not sentence_df.empty and sentence_embeddings.size > 0:
        ...
        # Split into train/test (80/20) for better evaluation
        ...
        arousal_history = arousal_model.fit(X_train, y_arousal_train, epochs=10, batch_size=32, verbose=1, validation_split=0.2)

        # Train valence model
        valence_model = create_mlp_regressor(input_dim)
        valence_history = valence_model.fit(X_train, y_valence_train, epochs=10, batch_size=32, verbose=1, validation_split=0.2)
        ...
        quadrant_accuracy = accuracy_score(true_quadrants, pred_quadrants)  # New: Classification accuracy
